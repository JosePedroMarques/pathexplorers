...
public class ColorPickingAgent implements Drawable {
	...
	public void walk() {
		...
		if(space.getObjectAt(this.x+xMove, this.y+yMove) == null && isInside(xMove,yMove)) {
			space.putObjectAt(this.x, this.y,null);
			this.x += xMove;
			this.y += yMove;
	...
	private boolean isInside(int xMove, int yMove) {
		
		if(this.x+xMove < 0 || this.x+xMove > space.getSizeX())
			return false;
		if(this.y+yMove < 0 || this.y+yMove > space.getSizeY())
			return false;
		return true;
	}
